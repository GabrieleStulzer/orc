...
class SingleShootingProblem:
    ...
    def compute_cost(self, y):
        ...
        if(np.any(np.isnan(y))):
            print("\t[Compute cost] The solution vector (U) contains NaN!")
        ...
        if(np.any(np.isnan(X))):
            print("\t[Compute cost] The state vector (X) contains NaN!")
    ...
    def compute_cost_w_gradient(self, y):
        ...
        if(np.any(np.isnan(y))):
            print("\t[Compute cost] The solution vector (U) contains NaN!", "red")
        # compute state trajectory X from control y
        U = y.reshape((self.N, self.nu))
        t0 = 0.0
        ...
        if(np.any(np.isnan(dXdU))):
            print("\t[Compute cost] The sensitivities (dXdU) contain NaN!")
...
#        jac = np.vstack((jac_run, jac_fin))
    ...
    def solve(self, y0=None, u_bounds=None, method='SLSQP', use_finite_diff=False, max_iter=300):
    ...
    def clbk(self, xk):
        ...
        if self.iter%100==0:
            ...
            if(np.any(np.isnan(xk))):
                print("\tThe solution vector (U) contains NaN!")
                return True
            ...
            for c in self.path_ineqs:
                ...
                if(v<=self.ineq_print_threshold):
                    print('\t Path ineq    %60s: %7.3f'%(c.name, v))
            for c in self.final_ineqs:
                v = np.min(self.last_values.__dict__[c.name])
                if(v<=0.0): color = 'red'
                else:       color = 'blue'
                if(v<=self.ineq_print_threshold):
                    print('\t Final ineq   %60s: %7.3f'%(c.name, v))
            for c in self.final_eqs:
