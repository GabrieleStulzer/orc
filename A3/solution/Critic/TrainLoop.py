...
def train_one_epoch(train_loader, optimizer, model, loss_fn, epoch_index, tb_writer, device):
    ...
    for i, data in enumerate(train_loader):
        ...
        # out = model(inputs)
        out = model(inputs.to(device, dtype=torch.float32))

        # Compute the loss and its gradients
        loss = loss_fn(out, labels.to(device, dtype=torch.float32))
        loss.backward()
...
def validate_one_epoch(val_loader, model, loss_fn, epoch_index, tb_writer):
    running_loss = 0.
    last_loss = 0.

    # Same as above, we use enumerate(val_loader) instead of
    # iter(val_loader) so that we can track the batch
    # index and do some intra-epoch reporting
